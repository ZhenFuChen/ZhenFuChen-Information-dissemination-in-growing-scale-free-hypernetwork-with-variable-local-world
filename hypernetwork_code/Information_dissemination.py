...
class InfoSpreading(object):
    ...
    def funRun1(self):
        # 导入模型
        self.importModel(1000,3,3,1,0.4,2)
        for beta in range(self.betaNum):
            print("\rβ="+str(beta/100),end="")
            self.sprSteadyState(beta/100, 0.1, 2, "avgHi")
    ...
    def funRun2(self):
        # 导入模型
        # 超边变量增长
        self.importModel(1000,3,3,1,0.2,2)
        self.spr_cp(0.1,0.1,0, "avgHi")
        self.importModel(3000,3,3,1,0.2,2)
    ...
    def funRun5(self):
        ...
        self.importModel(1000,3,3,1,0.2,2)
        self.spr_cp(0.1, 0.1, 0, "avgHi")
        self.importModel(1000,3,3,1,0.2,3)
        self.spr_cp(0.1, 0.1, 1, "avgHi")
        self.importModel(1000,3,3,1,0.2,4)
        self.spr_cp(0.1, 0.1, 2, "avgHi")
    ...
    def funRun6(self):
        ...
        # 超边变量增长
        self.importModel(1000, 3,3,1,0.2,2)
        self.spr(0.1, 0.1, 0, "maxHi")
        self.importModel(1000, 3,3,1,0.2,2)
        self.spr(0.1, 0.1, 1, "avgHi")
        self.importModel(1000, 3,3,1,0.2,2)
        self.spr(0.1, 0.1, 2, "minHi")
    ...
    def funRun7(self):
        # 导入模型
        # 超边变量增长
        self.importModel_BA(1000,3,3,3)
    ...
    def funRun8(self):
        # 导入模型
        # 超边变量增长
        self.importModel(1000,1,3,1,0.2,2)
        self.spr(0.1, 0.1, 0, "avgHi")
        self.importModel(1000,3,3,1,0.2,2)
        self.spr(0.1, 0.1, 1, "avgHi")
        self.importModel(1000,5,3,1,0.2,2)
    ...
    def funRun9(self):
        # 导入模型
        # 超边变量增长
        self.importModel(1000, 3, 1, 1, 0.2,2)
        self.spr_cp(0.1, 0.1, 0, "avgHi")
        self.importModel(1000, 3, 3, 1, 0.2,2)
        self.spr_cp(0.1, 0.1, 1, "avgHi")
        self.importModel(1000, 3, 5, 1, 0.2,2)
        self.spr_cp(0.1, 0.1, 2, "avgHi")
    ...
    def funRun10(self):
        ...
        self.importModel(1000, 3, 3, 1, 0.2, 2)
        self.spr(0.1, 0.1, 0, "avgHi")
        self.importModel(1000, 3, 3, 3, 0.2, 2)
        self.spr(0.1, 0.1, 1, "avgHi")
        self.importModel(1000, 3, 3, 5, 0.2, 2)
        self.spr(0.1, 0.1, 2, "avgHi")
    ...
    def funRun11(self):
        # 导入模型
        # 超边变量增长
        self.importModel(1000, 3, 3, 1, 0, 2)
        self.spr(0.1, 0.1, 0, "avgHi")
        self.importModel(1000, 3, 3, 1, 0.2, 2)
        self.spr(0.1, 0.1, 1, "avgHi")
        self.importModel(1000, 3, 3, 1, 0.4, 2)
        self.spr(0.1, 0.1, 2, "avgHi")
        self.importModel(1000, 3, 3, 1, 0.5, 2)
        self.spr(0.1, 0.1, 3, "avgHi")
        # self.importModel(1000, 3, 3, 1, 0.6, 2)
        # self.spr_cp(0.1, 0.1, 4, "avgHi")
        # self.importModel(1000, 3, 3, 1, 0.7, 2)
        # self.spr_cp(0.1, 0.1, 5, "avgHi")
        # self.dataExport()  # 数据存储
        self.imgDraw11()  # 绘图

    # 导入数据
    def importModel(self, N,m1,m2,m,al,n):  # 节点数(1000,3000,5000),超边数,θ值(实际值的十倍),模型(edge,node)
        self.nodesNum = N
        self.state = np.zeros((self.nodesNum, 2), int)  # 节点，状态(S(0),I(1))
        self.scenes = {}
        print("导入n" + str(N) +"m1"+str(m1)+"m2"+str(m2)+"m"+str(m)+'al' + str(int(al * 10)) + "_" + str(n) + 'version.txt' + "模型")
        # 超网络模型导入
        self.incidenceMatrix = np.genfromtxt(self.importPath+"n"+str(self.nodesNum)+"m1"+str(m1)+"m2"+str(m2)+"m"+str(m)+'al'+str(int(al * 10))+"_"+str(n)+"version.txt",delimiter=' ')
    ...
    def importModel_BA(self, N,m1,m2,m):  # 节点数(1000,3000,5000),超边数,θ值(实际值的十倍),模型(edge,node)
        self.nodesNum = N
        self.state = np.zeros((self.nodesNum, 2), int)  # 节点，状态(S(0),I(1))
        self.scenes = {}
    ...
    def spr_cp(self, beita, gama, n, select):
        ...
        for repeat in range(self.repeatNum):
            ...
            # 进行 t 个时间步传播
            for t in range(self.timeStep):
                current_I = list(self.IStateNodes) #当前时间步的I节点
                new_infected = []  # 新感染的节点
                recoverNodes = []  # 恢复节点列表
                if t >= 1:
                    for i in current_I:
                ...
                for node in current_I:
                    chosen_edge = random.choice(self.node_to_edge["v" + str(node)]) #随机取出I节点关联的一条超边
                    # print(chosen_edge)
                    for chosen_node in self.scenes[chosen_edge]:
                        idx = int(chosen_node[1:]) - 1
                        if self.state[idx][1] == 0:  # 处于S态的节点以一定概率感染
                            if random.randint(1, 100) <= 100 * beita:  # 感染
                                new_infected.append(idx + 1)  # I状态节点当中添加此节点编号

                #self.incidenceMatrix 行为节点，列为超边
                # 修改数据，更新状态
                self.IStateNodes = [i for i in self.IStateNodes if i not in recoverNodes]  # 删除恢复的节点
                self.IStateNodes.extend(new_infected) # 添加本次感染的节点
                self.IStateNodes = list(set(self.IStateNodes))  # 去除重复节点
    ...
    def imgDraw6(self):
        ...
        ax.legend(labels=["maxHD","avgHD","minHD"], ncol=1, fontsize=20)
        plt.xlabel("t", fontsize=25)
        plt.ylabel("ρ", fontsize=25)
        plt.xticks(fontsize=20)
        plt.yticks(fontsize=20)
    ...
    def imgDraw11(self):
        ...
        # ax.plot(t, yvalue4, color='#9467bd', linestyle='-', linewidth=1, marker='o', markersize=8,
        #         )
        # ax.plot(t, yvalue5, color='#8c564b', linestyle='-', linewidth=1, marker='*', markersize=5,
        #         )
        ax.legend(labels=[r"$α=0$", r"$α=0.2$", r"$α=0.4$", r"$α=0.5$"], ncol=1, fontsize=20)
        plt.xlabel("t", fontsize=25)
        plt.ylabel("ρ", fontsize=25)
        plt.xticks(fontsize=20)
        plt.yticks(fontsize=20)
