...
class hypergraph(object):
    def __init__(self):
        ...
        self.timestep = 1 # 模型构造的时间步t(不可变，根据节点数自动变更)
        self.n = 2 # 邻居节点的阶数，设为2
        self.alpha = 0.7 # α=0,0.2,0.4
        self.nodesNum = 1000 # 生成总节点个数
    ...
    def modelCreate(self):
        ...
        while self.lastNode < self.nodesNum:
            ...
            #((self.nodesNum - self.lastNode) / self.m1)
            while current_m < int(round((self.m * (self.timestep ** self.alpha)), 0)):  # 生成mt**alpha条超边
                self.lastEdge += 1
    ...
    def theoretical(self):
        ...
        for k in range(1, 35):
            Pk[k] = (self.m1+self.m2) * (self.m ** ((self.m1+self.m2)/(self.m2-self.m1 * self.alpha))) * (self.timestep ** (self.alpha * (self.m1+self.m2)/(self.m2-self.m1 * self.alpha))) / (self.m2-self.m1 * self.alpha) / (k**(((1-self.alpha)*self.m1+2*self.m2)/(self.m2-self.m1 * self.alpha)))
    ...
    def exportData(self):
        print("存储中...")
        file_path = self.dataSave+"n"+str(self.nodesNum)+"m1"+str(self.m1)+"m2"+str(self.m2)+"m"+str(self.m)+'al'+str(int(self.alpha*10))+"_"+str(self.n)+'version.txt'
